...
-(id)init{
    ...
    timerforIconImage = [NSTimer scheduledTimerWithTimeInterval:1.0 target:self selector:@selector(iconImage) userInfo:nil repeats:true];

    if (reviewHasBeenMade) {
        [self iconImage];
        [timerforIconImage invalidate];
    }
...
- (NSDictionary*)descriptionDictionary
{
    ...
    [self setObject:_toolName forKey:@"toolName"  inDictionary:dict];
...
- (UIImage*)iconImage  {
    ...
    /*
    if ([_toolName isEqualToString:@"Adjustment"] ) {
        return [UIImage imageNamed:@"adjustmentIcon"];
    }
    */

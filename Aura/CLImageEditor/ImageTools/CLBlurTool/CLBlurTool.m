...
- (void)setup  {
   ...
   // _thumnailImage = _originalImage;
    _thumnailImage = [_originalImage resize:self.editor.imageView.image.size];
    ...
    [_blurSlider setThumbImage:[CLImageEditorTheme imageNamed:[NSString stringWithFormat:@"%@/thumb", [self class]]] forState:UIControlStateNormal];
    [_blurSlider setThumbImage:[CLImageEditorTheme imageNamed:[NSString stringWithFormat:@"%@/thumb", [self class]]] forState:UIControlStateHighlighted];
    [_blurSlider setMinimumTrackTintColor:[UIColor whiteColor]];
    [_blurSlider setMaximumTrackTintColor:[UIColor darkGrayColor]];
    
    
   // _handlerView = [[UIView alloc] initWithFrame:self.editor.imageView.frame];
    //[self.editor.view addSubview:_handlerView];
   // [self setHandlerView];
...
{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        _blurImage = [_originalImage gaussBlur:_blurSlider.value];
...
- (UIImage*)buildResultImage:(UIImage*)image withBlurImage:(UIImage*)blurImage {
    ...
    NSLog(@"BlurIMG: %f - %f", blurImage.size.width, blurImage.size.height);
...
- (void)drawRect:(CGRect)rect {
    ...
    delay:1.0 options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionAllowUserInteraction
    animations:^{
    self.alpha = 0;
    } completion:^(BOOL finished) {
    }];
